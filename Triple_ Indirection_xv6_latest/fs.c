...
bmap(struct inode *ip, uint bn)
{
  ...
  if(bn < LEVEL2NINDIRECT){
      ...
      // quotient
	  level1index = bn/(NINDIRECT*NINDIRECT);

	  if((addr = a[level1index]) == 0)
	  {
		  a[level1index]= addr = balloc(ip->dev);
		 //log_write(bp); // Check for Performance
		  bwrite(bp);
	  }
	  brelse(bp);
	  bp = bread(ip->dev, addr);
	  ...
	  bn = bn - (level1index*NINDIRECT*NINDIRECT);
      // quotient
      level2index = bn/NINDIRECT;
      if((addr = a[level2index]) == 0)
      {
    	  a[level2index]= addr = balloc(ip->dev);
          //log_write(bp); // Check for Performance
    	  bwrite(bp);
      }
      brelse(bp);
